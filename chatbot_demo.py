...
def get_lecture_filter_indices(db, query):
    ...
    # 3. 월/연도가 특정되지 않은 경우: 질문에서 주제어를 뽑아 본문에 실제로
    ...
    # 4. 그래도 없으면 전체 lecture_summary 반환
    return [i for i, m in all_summaries]
...
def search_similar(db, query, n_results=5, source_filter=None):
    ...
    if is_lecture_q:
        ...
        # 상위권만 보면 실제 주제와 맞는 특강이 밀려날 수 있어 후보 범위를 넓힘.
...
- 모든 종교·전례 용어는 가톨릭 공식 용어 기준 (하느님/미사/고해성사/영성체/성모님 등). "하나님" 등 개신교식 표현 절대 금지
- 코너명은 반드시 "맹모닝 상담소" — "맥모닝"이라고 절대 쓰지 말 것 (참고자료 제목에 영어 오표기가 있어도 답변에서는 "맹모닝"으로만 표기)
